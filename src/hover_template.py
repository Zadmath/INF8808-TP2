...
def get_hover_template(name, mode):
    ...
    # Retour en fonction du mode
    hoverName=f'<span style= "font-family : \'Grenze Gotisch\'; font-size:24px;font-color:\'Black\'">{name}</span><br><br>'
    if mode==MODES['count']:
        return hoverName+'%{y} lines<extra></extra>'
    else:
        if mode==MODES['percent']:
            return hoverName+' %{y:.2f}% of lines <extra></extra>'
        else:
            return 'Probl̬me avec le choix du mode'
